...
#define NCOLOR normalize(vec3(0.1765, 0.1765, 0.3882))
...
vec4 minecraft_sample_lightmap(sampler2D lightMap, ivec2 uv) {
    ...
    light *= mix(vec4(1.0), vec4(0.0431, 0.0549, 0.0863, 1.0), torch * (1.0-sun));
    //shadows colors (NOT (torch OR sun))
    if (map.r == map.g && map.g == map.b) { //bluer shadows in overworld
        light *= mix(vec4(1.0), vec4(0.1608, 0.1608, 0.6078, 1.0), max(1.0 - (torch + sun), 0.0));
    } else if (map.r > map.g) { //redder shadows in nether
        light *= mix(vec4(1.0), vec4(0.7608, 0.1373, 0.1373, 1.0), max(1.0 - (torch + sun), 0.0));
    } else { //purple shadows in end
        light *= mix(vec4(1.0), vec4(0.5529, 0.2824, 0.5529, 1.0), max(1.0 - (torch + sun), 0.0));
    }
    return light;
}